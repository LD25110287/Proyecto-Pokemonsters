...
MenuScreen::MenuScreen()
    : window(sf::VideoMode(800, 600), "Pokemonsters", sf::Style::Close),
      ...
      launchGame(false),
      detailIndex(-1),
      // Inicializa el nivel mostrado (0-10) a partir del volumen real
      // que ya está sonando en AudioManager (0-100)
      volumeLevel(static_cast<int>(AudioManager::getVolume() / 10.f + 0.5f))
{
    ...
    // La música de fondo la maneja AudioManager (main.cpp ya la inició).
    // MenuScreen NO carga ni reproduce música propia.
}
    ...
    if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
    {
        ...
        if (state == MenuState::SETTINGS)
        {
            ...
            if (btnDecrease.getGlobalBounds().contains(m))
            {
                ...
                {
                    volumeLevel--;
                    // AudioManager espera 0-100, volumeLevel está en 0-10
...
void MenuScreen::drawSettingsScreen()
{
    ...
    // ── Barra de volumen segmentada de 10 bloques (más delgada) ──────────────
    // Botones [-]/[+] son 50x50 en y=240 (centro vertical = 265).
    // Bloques de 18px de alto, centrados con los botones: barY = 265 - 18/2
    float startBarX  = 140.f;
    float blockWidth = 28.f;
    float blockHeight = 18.f;
    float spacing    = 4.f;
    float barY       = 256.f;
